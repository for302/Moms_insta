...
export default function ContentProgressModal() {
  const { generationProgress, closeProgressModal } = useContentStore();

  if (!generationProgress.isOpen) return null;
  ...
  const allDone = generationProgress.steps.every(
    (step) => step.status === "done" || step.status === "error"
  );

  const hasError = generationProgress.steps.some((step) => step.status === "error") || !!generationProgress.error;
          ...
          {generationProgress.error && (
            <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm text-red-700">{generationProgress.error}</div>
              </div>
            </div>
          )}
